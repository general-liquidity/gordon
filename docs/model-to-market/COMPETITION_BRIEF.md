...
- **Order-book matching model**, *not* dealing-desk. Limit orders rest as liquidity and fill by **queue position + available liquidity**; **partial fills** occur when volume is limited. **A limit order is NOT guaranteed to fill just because the market touches your price** (Duncan, Discord) — fills depend on liquidity, order size, and queue position; **there is no counterparty order flow from other participants** (matching is per-account vs the simulated book, see below). ⇒ **we trade TAKER (market orders via the reconcile loop), sidestepping resting-order fill uncertainty** — a resting limit would add un-modelled fill risk and could silently leave the book un-hedged.
