...
- **Order-book matching model**, *not* dealing-desk. **RESTING LIMIT ORDERS ARE ALLOWED (Duncan, Discord, confirmed)** — they rest as liquidity and fill by **queue position + available liquidity**; **partial fills** occur when volume is limited; **a limit is NOT guaranteed to fill just because the market touches your price** (fills depend on liquidity, size, queue). No counterparty flow from other participants (per-account matching vs the simulated book).
  ...
  - **Plan:** go live **TAKER** (safe, hedged, rehearsed); **test maker fills in the setup window**; build/switch to maker (the new top-priority build) if live fills validate. Adaptation is allowed, so we have rounds to switch.
...
- **Costs: NO commission, and NO swap / overnight financing (Duncan, Discord, CONFIRMED).** Friction = **spread + slippage + market impact only** — and the spread flips to a *rebate* if we make. No flatten-over-carry pressure; holds are free. Slippage & market impact **vary with conditions, depth, and liquidity** (dynamic, not fixed) — reinforces depth-aware sizing.
