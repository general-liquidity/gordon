...
Why this shape fits *this* contest: it is a relative tournament with attrition — a **~5-day leveraged live window** (21 Jun 22:00 → 26 Jun 22:00), three nightly elimination rounds (22/23/24 Jun), then a **blind** 2-day final for the Top 100 (§5, §8). Crucially, **equity carries over between rounds — it does not reset** (§2): a single blow-up is *permanent*, and **forced liquidation at the 30% margin-level stop-out is instant elimination** (§14). That makes the format reward **survive-and-compound over variance** even harder than a per-round reset would — a system that *cannot* wipe out has a structural ranking edge before the strategy contributes a single basis point. The early rounds are *observable* (peer logs, positions, leaderboard at 5-minute latency, §8), so the rank-tracker can time the variance decision on **real** standing; the final is blinded, so that call is pre-committed. And — as §6 below argues honestly — in a likely luck-dominated return tournament, **disciplined execution, survival, and the governance architecture are the defensible edges**.
...
1. **CORE (always on) — a dollar-neutral relative-value reversion book.** This is the Best-Sharpe vehicle. A key empirical finding drove it: across TSMOM, a mean-reversion scalper, and pairs, **only the *dollar-neutral* book produced a smooth curve** — smoothness comes from neutrality, not signal type (a directional book carries beta that gets hammered on a correlated trend; tuning brackets made it *worse*). The core trades the log-ratio z-score of every within-cluster pair (10 crypto-pair spreads + XAU/XAG): neutral → smooth, breadth → it clears the §17 ≥30-trade floor (2083 trades in rehearsal) where a single cointegrated pair could not. Rehearsal curve: per-bar return std **9.7e-4**, max drawdown **2.1%** — the low-variance, low-DD shape the Sharpe (10%) + Drawdown (15%) ranks reward. The 17 Jun cost sweep tightened the live candidate toward **low-churn discrete hysteresis** (`lookback 96`, `entryZ 2.5`, `exitZ 0.75`) when spreads are wide; crypto-only was slightly cleaner, but the final dial is decided from live spreads. The field is *uncontested*: ~500 entrants gambling for the return rank are tanking their own Sharpes.
...
- **Path-risk Monte Carlo** (`pathRiskMonteCarlo.ts`) — *"a backtest is one path; we read 10,000."* Block-bootstrap (preserves vol clustering) + GARCH(1,1) resampling, not the vanilla shuffle that under-states tail risk. The old high-churn stress script is deliberately sobering: naive IID reports a 17% 95th-pct drawdown, while clustering-aware GARCH reports **23%** (1.37× wider). The 17 Jun low-churn discrete sweep is the candidate we would actually consider at wide spreads: 5-day p95 DD was **<2%** at 3%/leg while still clearing the ≥30-trade floor. We size for the 5th-percentile path, not the median — the discipline a quant desk uses to decide *trade-it-or-not*.
...
- So we built toward what *is* consistent and measurable:
  - **A barbell, chosen by tournament theory** — a dollar-neutral relative-value reversion CORE (the uncontested Best-Sharpe vehicle: smooth curve, 2.1% rehearsal DD, ≥30 trades; low-churn discrete candidate selected by live spreads) + a ring-fenced, liquidation-bounded lottery SLEEVE deployed only post-cut-and-lagging. The dominated middle is avoided by design.
  - **Express AND protect alpha as machinery** — ring-fence accounting, a first-passage liquidation guard, a bang-bang rank tracker, and a block-bootstrap+GARCH path-risk Monte Carlo (the old high-churn stress shows 23% clustered-tail DD; the measured low-churn candidate keeps 5-day p95 DD <2% at 3%/leg).
