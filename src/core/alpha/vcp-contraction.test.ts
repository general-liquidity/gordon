...
import { analyzeVcpContraction, formatVcpContraction, type VcpCandle } from "./vcp-contraction.ts";
...
describe("analyzeVcpContraction", () => {
  test("insufficient_data with too few candles", () => {
    const bars = Array(3)
      .fill(0)
      .map(() => makeCandle(100, 102, 99, 101, 1000));
  ...
  test("classic VCP: body+range+volume all shrinking → spring_ready", () => {
    const baseline = Array(20)
      .fill(0)
      .map(() => makeCandle(100, 105, 95, 100, 10000));
  ...
  test("only volume contracting → mixed", () => {
    const baseline = Array(20)
      .fill(0)
      .map(() => makeCandle(100, 102, 99, 101, 10000));
  ...
  test("body+range contracting but volume rising → contracting (not spring_ready)", () => {
    const baseline = Array(20)
      .fill(0)
      .map(() => makeCandle(100, 105, 95, 100, 1000));
    const tail: VcpCandle[] = [];
    const widths = [2.0, 1.5, 1.0, 0.6, 0.3];
  ...
  test("tight range required for spring_ready", () => {
    ...
    expect(r.windowRangeOverLow).toBeGreaterThan(0.1);
  ...
  test("respects custom thresholds", () => {
    const baseline = Array(20)
      .fill(0)
      .map(() => makeCandle(100, 102, 99, 100, 10000));
    const tail: VcpCandle[] = [];
  ...
  test("zero-mean volume window handles gracefully", () => {
    const bars = Array(20)
      .fill(0)
      .map(() => makeCandle(100, 102, 99, 101, 0));
    const r = analyzeVcpContraction(bars);
