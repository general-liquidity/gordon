...
export function calculateRiskReward(plan: Plan): number {
  ...
  const reward = multiplier * (tp1Price - entryPrice);
 ...
 */
export function calculateAllocationPercent(plan: Plan, portfolioValue: number): number {
  if (portfolioValue <= 0) {
    return 0;
  }
...
function validateStructure(plan: Plan, errors: string[]): void {
  ...
  // Stop loss presence and direction check.
  ...
  } else if (plan.entry.price !== null && plan.direction === "short") {
    if (plan.stopLoss.price <= plan.entry.price) {
      errors.push(
        `Stop loss (${plan.stopLoss.price}) must be above entry price (${plan.entry.price}) for short positions.`,
      );
    }
  }
  ...
  if (!plan.takeProfit || plan.takeProfit.length === 0) {
    ...
    const tpPercentSum = plan.takeProfit.reduce((sum, tp) => sum + tp.percentToSell, 0);
    ...
    // Validate TP prices lie in the profit direction.
    ...
    if (plan.entry.price !== null && plan.direction === "short") {
      for (let i = 0; i < plan.takeProfit.length; i++) {
        const tp = plan.takeProfit[i];
        if (tp && tp.price >= plan.entry.price) {
          errors.push(
            `Take profit level ${i + 1} (${tp.price}) must be below entry price (${plan.entry.price}) for short positions.`,
          );
        }
      }
    }
  ...
  if (plan.dca && plan.dca.length > 0) {
    const dcaPercentSum = plan.dca.reduce((sum, level) => sum + level.percentOfAllocation, 0);
    ...
    // Validate DCA prices lie in the adverse direction.
    if (plan.entry.price !== null && plan.direction === "long") {
    ...
    if (plan.entry.price !== null && plan.direction === "short") {
      for (let i = 0; i < plan.dca.length; i++) {
        ...
        if (dca && dca.price <= plan.entry.price) {
          errors.push(
            `DCA level ${i + 1} (${dca.price}) must be above entry price (${plan.entry.price}) for short positions.`,
          );
        }
      }
    }
...
function validateRisk(
  ...
  const maxAllocationAmount = config.preferences.maxAllocationPerTrade * portfolio.totalValue;
  const minCashReserve = config.preferences.cashReservePercent * portfolio.totalValue;
 ...
 */
function generateWarnings(plan: Plan, portfolio: PortfolioState, warnings: string[]): void {
  // High volatility warning (stop > 10%)
  if (plan.entry.price !== null) {
    const stopDistance = Math.abs(plan.entry.price - plan.stopLoss.price) / plan.entry.price;

    if (stopDistance > HIGH_VOLATILITY_THRESHOLD && stopDistance <= MAX_STOP_LOSS_PERCENT) {
      warnings.push(
  ...
  const riskReward = calculateRiskReward(plan);
  if (riskReward >= MIN_RISK_REWARD_RATIO && riskReward < LOW_REWARD_THRESHOLD) {
    warnings.push(
 ...
 */
function validateGrid(plan: Plan, errors: string[], warnings: string[]): void {
  ...
  if (grid.levels.length < 3 || grid.levels.length > 7) {
    errors.push(`Grid must have 3-7 levels. Current: ${grid.levels.length} levels.`);
  }
