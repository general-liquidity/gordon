...
export function validatePlaybook(playbook: unknown): ProtocolValidationResult {
  ...
  if (tpTotalPercent > 100) {
    errors.push(`exit.take_profit levels sum to ${tpTotalPercent}% — cannot exceed 100%`);
  }

  // Stop loss value should make sense (not zero or negative)
  if (pb.exit.stop_loss.value <= 0) {
    errors.push(`exit.stop_loss.value must be positive (got ${pb.exit.stop_loss.value})`);
  }
  ...
  for (let i = 0; i < pb.exit.take_profit.length; i++) {
    ...
    if (tp.level <= 0) {
      errors.push(`exit.take_profit[${i}].level must be positive (got ${tp.level})`);
    }
  ...
  if (pb.exit.trailing_stop?.enabled) {
    if (
      pb.exit.trailing_stop.activation === undefined &&
      pb.exit.trailing_stop.distance === undefined
    ) {
      warnings.push("exit.trailing_stop is enabled but neither activation nor distance is set");
    }
  }
  ...
  if (pb.exit.take_profit.length === 0) {
    warnings.push("exit.take_profit has no levels defined — consider adding at least one target");
  }

  // No entry filters
  if (!pb.entry.filters || pb.entry.filters.length === 0) {
    warnings.push("entry.filters is empty — consider adding filters to reduce false signals");
  }
