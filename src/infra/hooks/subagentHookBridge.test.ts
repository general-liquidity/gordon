...
import { beginSubagentHook, endSubagentHook, resetSubagentHookBridgeForTests } from "./subagentHookBridge.ts";
...
describe("subagent hook bridge", () => {
  test("pairs start and stop with one stable id", async () => {
    const seen: unknown[] = [];
    registerHook({ id: "start", point: "SubagentStart", handler: (payload) => { seen.push(payload); return { action: "allow" }; } });
    registerHook({ id: "stop", point: "SubagentStop", handler: (payload) => { seen.push(payload); return { action: "allow" }; } });
    const start = await beginSubagentHook({ key: "k", id: "run-1", type: "researcher", parent: "Gordon" });
  ...
  test("a start block prevents activation and therefore emits no stop", async () => {
    let stopped = 0;
    registerHook({ id: "deny", point: "SubagentStart", handler: () => ({ action: "block", reason: "disabled" }) });
    registerHook({ id: "stop", point: "SubagentStop", handler: () => { stopped += 1; return { action: "allow" }; } });
