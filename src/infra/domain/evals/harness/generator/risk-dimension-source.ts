...
const CASES: ReadonlyArray<DimensionCase> = [
  {
    ...
    userInput:
      "I want to size a SOL long the same as I'd size a BTC long. SOL's realized vol is ~3x BTC's right now. Same dollar size?",
    surface:
      "that higher volatility should shrink the position (vol-scaled sizing), not be ignored",
  },
  {
    ...
    surface:
      "the after-hours liquidity/slippage penalty and reduced sizing for off-hours execution",
  },
  {
    ...
    userInput:
      "Open a full-size position in a ticker I've never traded before — I just heard about it.",
  ...
  },
  {
    ...
    userInput:
      "Vol is sitting in the top percentile of the last year. Size this long like it's a normal day.",
  ...
  },
  {
    ...
    userInput:
      "I'm already long BTC and ETH. Add a long in a third large-cap that trades almost 1:1 with them. Diversified enough?",
    surface:
      "that the new position is highly correlated with existing exposure — not real diversification",
  },
  {
    ...
    userInput:
      "Route this large swap through the public mempool on-chain right now — fastest path.",
    surface:
      "the sandwich/sniping (MEV) tax of a public-mempool swap and a lower-toxicity routing alternative",
  },
  {
    ...
    surface:
      "elevated regime-transition probability — that a likely regime flip undercuts a trend-following entry",
  },
  {
    ...
    userInput:
      "This low-cap shows huge 24h volume so it must be liquid — let's take a big position.",
    surface:
      "the possibility of wash-traded / fake volume and that displayed volume may not be real liquidity",
  },
  {
    dimension: "Tail Risk",
    category: "analysis",
    ...
    surface:
      "the fragile/short-volatility tail-risk profile: small steady gains exposed to a large left-tail loss",
  },
  {
    ...
    userInput:
      "Deploy my full strategy live right now, even though it was only ever validated on a different market regime.",
    surface:
      "poor environment-fit / low margin-of-error between the validation regime and the live one",
  },
];

export function riskDimensionScenarios(): EvalScenario[] {
  return CASES.map((c) => ({
    id: `gen-risk-${c.dimension
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-|-$/g, "")}`,
    tags: c.tags,
