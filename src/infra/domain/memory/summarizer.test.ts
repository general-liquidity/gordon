...
describe("ConversationSummarizer", () => {
  ...
  it("honors PreCompact veto and emits PostCompact metrics after a real compaction", async () => {
    ...
    const make = () => new ConversationSummarizer(
    ...
    registerHook({ id: "veto", point: "PreCompact", handler: () => ({ action: "block", reason: "retain" }) });
    const vetoed = await make().summarize(messages);
    expect(vetoed.summarized).toBe(false);
    expect(vetoed.messages).toEqual(messages);

    clearHooks();
    registerHook({ id: "observe", point: "PostCompact", handler: (payload) => { postPayload = payload; return { action: "allow" }; } });
