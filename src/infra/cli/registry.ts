...
export const CLI_REGISTRY: CLIEntry[] = [
  {
    ...
    name: "Boba CLI",
    description:
      "AI agent trading toolkit — DEX swaps, limit orders, DCA, TWAP, perps, prediction markets, portfolio tracking, copy trading, honeypot detection.",
    ...
    commands: [
      {
        ...
        example: "boba swap --from ETH --to USDC --amount 0.5 --chain base",
      },
      {
        command: "boba limit",
        description: "Place limit order",
        example: "boba limit --buy ETH --at 3000 --spend 500 --chain ethereum",
      },
      {
        command: "boba dca",
        description: "Dollar-cost average",
        example: "boba dca --buy ETH --amount 100 --every week --for 12",
      },
      {
        command: "boba twap",
        description: "Time-weighted average price order",
        example: "boba twap --sell ETH --amount 10 --over 24h",
      },
      {
        command: "boba perps",
        description: "Perpetual futures (Hyperliquid)",
        example: "boba perps --long BTC --size 1000 --leverage 5x",
      },
      {
        command: "boba predict",
        description: "Prediction markets (Polymarket)",
        example: "boba predict --market 'BTC 100k' --position yes --amount 50",
      },
      {
        command: "boba portfolio",
        description: "Portfolio overview",
        example: "boba portfolio --chain all",
      },
      {
        command: "boba track",
        description: "Track KOL/whale wallets",
        example: "boba track --wallet 0x... --alerts on",
      },
      {
        command: "boba copy",
        description: "Copy trading",
        example: "boba copy --wallet 0x... --max-size 100",
      },
      {
        command: "boba scan",
        description: "Token scanner + honeypot detection",
        example: "boba scan --token 0x... --chain base",
      },
      {
        command: "boba launch",
        description: "Token launch tools",
        example: "boba launch --name MyToken --chain base",
      },
      { command: "boba mcp", description: "Start MCP server mode", example: "boba mcp" },
    ],
    nativeCoverage: "partial",
    nativeTools: ["place_market_order", "place_limit_order", "get_portfolio", "scan_market"],
    credentials: [
      {
        env: "BOBA_API_KEY",
        required: false,
        description: "Optional — some features work without",
      },
  ...
  },
  {
    ...
    commands: [
      {
        command: "hummingbot start",
        description: "Start a strategy",
        example: "hummingbot start --strategy pure_market_making",
      },
  ...
  },
  {
    ...
    commands: [
      {
        command: "lean init",
        description: "Initialize a new Lean project in the current directory",
      },
      {
        command: "lean backtest",
        description: "Run a backtest locally",
        example: 'lean backtest "My Project"',
      },
    ...
    credentials: [
      {
        env: "QUANTCONNECT_USER_ID",
        required: false,
        description: "Required only for cloud commands (lean cloud *)",
      },
      {
        env: "QUANTCONNECT_API_TOKEN",
        required: false,
        description: "Required only for cloud commands",
      },
    ],
    ...
    pricing: "freemium",
    pricingNote:
      "CLI is free + open source (Apache 2.0). Local backtests free; QC Cloud has paid tiers for live trading and large datasets. Pairs with the 'quantconnect' MCP server for AI-driven strategy authoring.",
  },
  {
    id: "alpaca-cli",
    name: "Alpaca CLI",
  ...
  },
  {
    ...
    name: "Kraken CLI",
    description:
      "First open-source CLI built for AI agents — 134 commands for crypto, stocks, forex, and derivatives. Paper trading, WebSocket streaming, dead man's switch.",
    installCommand:
    ...
    commands: [
      {
        ...
        example: "kraken buy --pair BTC/USD --amount 0.01 --type market",
      },
      {
        command: "kraken sell",
        description: "Place a sell order",
        example: "kraken sell --pair ETH/USD --amount 1 --type limit --price 4000",
      },
      ...
      {
        command: "kraken ticker",
        description: "Live ticker data",
        example: "kraken ticker --pair BTC/USD",
      },
      {
        command: "kraken ohlc",
        description: "OHLC candle data",
        example: "kraken ohlc --pair BTC/USD --interval 60",
      },
      ...
      {
        command: "kraken cancel-after",
        description: "Dead man's switch — auto-cancel orders if process halts",
        example: "kraken cancel-after --timeout 60",
      },
      {
        command: "kraken --validate",
        description: "Simulate execution without placing orders (dry run)",
      },
      {
        command: "kraken --paper",
        description: "Paper trading against live prices, no credentials needed",
      },
      {
        command: "kraken mcp",
        description: "Start MCP server mode — exposes all 134 commands via MCP",
      },
    ],
    nativeCoverage: "partial",
    nativeTools: [
    ...
    credentials: [
      {
        env: "KRAKEN_API_KEY",
        required: false,
        description: "Not needed for market data or paper trading",
      },
    ...
    pricing: "free",
    pricingNote:
      "Open source (MIT). Paper trading free, no credentials. Live trading requires Kraken account.",
  },
  {
    id: "openbb",
    name: "OpenBB CLI",
    description:
      "All-in-one investment research terminal — 40+ data providers, screening, fundamentals, economic data, charting. 62K+ GitHub stars.",
    installCommand: "pip install openbb",
    bin: "openbb",
    commands: [
      { command: "openbb", description: "Launch interactive research terminal" },
      {
        command: "openbb economy",
        description: "Macroeconomic data (GDP, CPI, rates)",
        example: "openbb economy gdp --country US",
      },
      {
        command: "openbb equity",
        description: "Stock fundamentals + technicals",
        example: "openbb equity price --symbol AAPL",
      },
      {
        command: "openbb crypto",
        description: "Crypto market data",
        example: "openbb crypto price --symbol BTC",
      },
      {
        command: "openbb forex",
        description: "Forex pairs and rates",
        example: "openbb forex quote --symbol EURUSD",
      },
      {
        command: "openbb options",
        description: "Options chains and Greeks",
        example: "openbb options chains --symbol AAPL",
      },
    ...
    credentials: [
      {
        env: "OPENBB_TOKEN",
        required: false,
        description: "Optional — enhances some data providers",
      },
    ],
  ...
  },
  {
    ...
    description:
      "Local algorithmic trading development — write strategies in Python/C#, backtest against QuantConnect's data library, optimize, deploy to live.",
    installCommand: "pip install lean",
    bin: "lean",
    commands: [
      ...
      {
        command: "lean backtest",
        description: "Run a local backtest",
        example: "lean backtest 'My Strategy'",
      },
      {
        command: "lean optimize",
        description: "Parameter optimization",
        example: "lean optimize 'My Strategy'",
      },
      {
        command: "lean live",
        description: "Deploy to live trading",
        example: "lean live 'My Strategy' --brokerage alpaca",
      },
    ...
    nativeCoverage: "partial",
    nativeTools: ["backtest_strategy", "optimize_strategy", "compare_backtests"],
    credentials: [
      {
        env: "QC_USER_ID",
        required: false,
        description: "QuantConnect user ID (for cloud features)",
      },
    ...
    pricing: "freemium",
    pricingNote:
      "Free for local backtesting. QuantConnect cloud: free tier + paid plans from $8/mo.",
  },
  {
    id: "jupiter",
    name: "Jupiter CLI",
    description:
      "Official Jupiter DEX CLI — spot swaps, perps (leveraged longs/shorts), lending, prediction markets, token verification. Dominant Solana DEX aggregator.",
    ...
    commands: [
      {
        ...
        example: "jup swap --from SOL --to USDC --amount 1",
      },
      { command: "jup portfolio", description: "View portfolio holdings" },
      {
        command: "jup perps open",
        description: "Open leveraged position",
        example: "jup perps open --long SOL --size 100 --leverage 3x",
      },
      { command: "jup perps close", description: "Close perpetual position" },
      {
        command: "jup lend deposit",
        description: "Deposit into lending pool",
        example: "jup lend deposit --token USDC --amount 1000",
      },
    ...
    credentials: [
      {
        ...
        required: true,
        description: "External Jupiter CLI wallet (not Gordon native)",
      },
  ...
  },
  {
    ...
    name: "Visa CLI",
    description:
      "Agent payments — let Gordon pay for premium data feeds, APIs, and services on-demand via Visa.",
    ...
    commands: [
      ...
      {
        command: "visa-cli pay",
        description: "Pay for a service",
        example: "visa-cli pay https://api.example.com/data",
      },
    ...
    credentials: [
      {
        env: "VISA_CLI_TOKEN",
        required: true,
        description: "GitHub OAuth enrollment required (closed beta)",
      },
    ],
  ...
  },
  {
    ...
    name: "Stripe CLI",
    description:
      "Payment processing — create charges, manage subscriptions, send payouts, listen to webhooks from the terminal.",
    ...
    commands: [
      { command: "stripe login", description: "Authenticate with Stripe account" },
      {
        ...
        example: "stripe listen --forward-to localhost:3000/webhook",
      },
      {
        command: "stripe trigger",
        description: "Trigger test events",
        example: "stripe trigger payment_intent.succeeded",
      },
      {
        command: "stripe charges create",
        description: "Create a charge",
        example: "stripe charges create --amount 1000 --currency usd",
      },
  ...
  },
  {
    ...
    description:
      "Official agent-skill installer maintained by Vercel Labs (skills.sh). Hub-agnostic — pulls SKILL.md files from any Git URL (binance-skills-hub, vercel-labs/agent-skills, custom org repos) into the standard agent-skills directory so Gordon's skill-loader picks them up alongside its own. This is how Binance intends their Skills Hub to be consumed; we don't wrap it, we register the canonical installer.",
    npmPackage: "skills",
    bin: "skills",
    commands: [
      {
        command: "skills add <repo>",
        description: "Install a skill pack from a GitHub repo URL or org/name shortform",
        example: "npx skills add binance/binance-skills-hub",
      },
      {
        command: "skills add (alt)",
        description: "Full URL form for non-GitHub sources or community packs",
        example: "npx skills add https://github.com/vercel-labs/agent-skills",
      },
    ...
    pricing: "free",
    pricingNote:
      "Free, open-source (github.com/vercel-labs/skills, 16K+ stars). Skills installed via this tool inherit the underlying repo's license — Binance Skills Hub is MIT.",
  },
  {
    id: "binance-cli",
    name: "Binance CLI",
    description:
      "Official Binance CLI — full Spot, Futures (USDS-M / COIN-M), Options, Margin, Convert, Earn, Copy-Trading, Sub-Account, VIP Loan, Mining, Pay, Gift Card, Algo, Alpha, Dual Investment, and more. 25+ product groups, 200+ subcommands.",
    npmPackage: "@binance/binance-cli",
    bin: "binance-cli",
    commands: [
      {
        command: "binance-cli profile create",
        description: "Create an auth profile (one-time setup)",
        example:
          "binance-cli profile create --name main --api-key <KEY> --api-secret <SECRET> --env prod",
      },
      {
        command: "binance-cli profile select",
        description: "Switch active profile",
        example: "binance-cli profile select --name main",
      },
      {
        command: "binance-cli spot",
        description: "Spot trading (orders, balances, history, market data)",
        example: "binance-cli spot account-info",
      },
      {
        command: "binance-cli futures-usds",
        description: "USDS-margined perpetuals + delivery futures",
        example: "binance-cli futures-usds account-info",
      },
      ...
      {
        command: "binance-cli convert",
        description: "Instant convert between assets",
        example: "binance-cli convert quote --fromAsset USDT --toAsset BTC --fromAmount 100",
      },
      { command: "binance-cli simple-earn", description: "Flexible/locked savings, staking" },
      {
        command: "binance-cli wallet",
        description: "Wallet operations — deposits, withdrawals, transfers",
      },
      ...
      {
        command: "binance-cli request",
        description:
          "Generic signed request — escape hatch for endpoints not in the dedicated subcommands",
        example: "binance-cli request GET /api/v3/account --signed",
      },
    ],
    nativeCoverage: "partial",
    nativeTools: [
    ...
    credentials: [
      {
        ...
        required: false,
        description:
          "Set via 'binance-cli profile create' or env var. Read-only queries can run unauthenticated.",
      },
      {
        env: "BINANCE_SECRET_KEY",
        required: false,
        description: "Pair with BINANCE_API_KEY for authenticated calls.",
      },
      {
        env: "BINANCE_API_ENV",
        required: false,
        description: "prod | demo | testnet (default: prod)",
      },
      {
        env: "BINANCE_SPOT_BASE_PATH",
        required: false,
        description: "Custom Spot base URL (testnet, regional, etc.)",
      },
    ],
  ...
  },
  {
    ...
    description:
      "HTTP 402 agent-to-agent payments — pay per API call, tool use, or data request. The open standard for the agentic economy.",
    ...
    commands: [
      ...
      {
        command: "mpp pay",
        description: "Pay for a resource",
        example: "mpp pay https://api.data-provider.com/prices",
      },
      ...
      {
        command: "mpp config",
        description: "Configure payment methods (Stripe, stablecoins, Lightning)",
      },
    ],
    nativeCoverage: "none",
    credentials: [
      {
        env: "MPP_WALLET_KEY",
        required: true,
        description: "Wallet key for payments (Stripe, stablecoin, or Lightning)",
      },
    ],
    ...
    pricing: "free",
    pricingNote:
      "Open protocol. Transaction fees depend on payment rail (Stripe, stablecoins, Lightning).",
  },
];
...
export async function isCLIInstalled(bin: string): Promise<boolean> {
  // Registry entries are executable names, never commands or paths. Keeping
  // this boundary narrow also prevents a caller from smuggling shell syntax
  // into what used to be an interpolated `where ${bin}` / `which ${bin}`.
  if (!/^[A-Za-z0-9][A-Za-z0-9._+-]*$/.test(bin)) return false;

  const resolver = process.platform === "win32" ? "where" : "which";
  const result = spawnSync(resolver, [bin], {
    stdio: "ignore",
    timeout: 3_000,
    windowsHide: true,
  });
...
export function formatCLIRegistryForPrompt(): string {
  ...
  for (const cli of CLI_REGISTRY) {
    const coverage =
      cli.nativeCoverage === "full"
        ? "(also native)"
        : cli.nativeCoverage === "partial"
          ? "(partial native)"
          : "";
