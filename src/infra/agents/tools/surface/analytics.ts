...
function dispatchIndicator(
  ...
  switch (indicator) {
    ...
    case "tsi":
      return calculateTSI(closes, {
        ...(typeof params.longPeriod === "number" && { longPeriod: params.longPeriod }),
        ...(typeof params.shortPeriod === "number" && { shortPeriod: params.shortPeriod }),
        ...(typeof params.signalPeriod === "number" && { signalPeriod: params.signalPeriod }),
      });
    case "mama":
      return computeMAMA({
...
export const computeIndicatorTool = createTool({
  ...
  description: [
    ...
    "OVERNIGHT/INTRADAY: overnight_intraday (candles) — splits returns into overnight (open/prevClose) vs intraday (close/open) legs, cumulates each; flags overnight-premium anomaly + the Knuteson buy-high-sell-low manipulation signature (overnight strongly + AND intraday strongly −). params { suspiciousThresholdPct? 10 }",
    "TSI: tsi (True Strength Index — William Blau double-smoothed momentum oscillator: 100·EMA_s(EMA_r(Δclose))/EMA_s(EMA_r(|Δclose|)); params { longPeriod? 25, shortPeriod? 13, signalPeriod? 13 }; bounded [-100,100], >0 bullish momentum, TSI/signal crossover = MACD-like trigger. Returns line + signal + zeroLine + crossover. Distinct from RSI (avg gain/loss); works on any series incl. FX/metals/crypto)",
...
export const computeRiskTool = createTool({
  id: "compute_risk",
  description: [
    "Run the 15-dimension pre-trade risk classifier (8 base + 7 optional when",
    "hedge-fund-grade inputs are available) on a proposed trade or hypothetical.",
    ...
    "",
    "Base dimensions: position size, concentration, drawdown proximity, daily loss",
    "budget, trade frequency, volatility, market hours, asset familiarity.",
    "Optional (when data supplied): vol-adjusted sizing, correlation risk, venue",
    "MEV exposure, regime transition risk, fake liquidity, margin of error, tail risk.",
    "",
