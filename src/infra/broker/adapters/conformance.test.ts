...
import { WebullAdapter } from "./webull.ts";
...
runBrokerConformanceSuite({
  name: "webull",
  expectedBrokerId: "webull",
  expectedDisplayName: "Webull",
  credentials: {
    apiKey: "webull-key",
    apiSecret: "webull-secret",
    paper: true,
    baseUrl: "https://webull.test",
    dataBaseUrl: "https://webull.test",
  },
  createAdapter: (credentials) => new WebullAdapter(credentials),
  route: (request) => {
    const { pathname, searchParams } = request.url;

    if (pathname === "/openapi/account/list" && request.method === "GET") {
      expect(request.headers.get("x-app-key")).toBe("webull-key");
      expect((request.headers.get("x-signature") || "").length).toBeGreaterThan(0);
      return json({
        code: 0,
    ...
    if (pathname === "/openapi/account/balance" && request.method === "GET") {
      expect(searchParams.get("accountId")).toBe("wb-acc-1");
      return json({
        code: 0,
        data: {
          accountId: "wb-acc-1",
          ...
          currency: "USD",
          cash: "5200.45",
          buyingPower: "11000.00",
          portfolioValue: "14000.25",
    ...
    if (pathname === "/openapi/market-data/trading/clock" && request.method === "GET") {
      return json({
        code: 0,
        data: {
          timestamp: "2026-03-05T14:30:00Z",
          isOpen: true,
          nextOpen: "2026-03-06T14:30:00Z",
          nextClose: "2026-03-05T21:00:00Z",
        },
      });
    }

    if (pathname === "/openapi/account/positions" && request.method === "GET") {
      return json({
        code: 0,
        data: [{
          symbol: "AAPL",
          ...
          avgEntryPrice: "200",
          unrealizedPl: "15.6",
          unrealizedPlPercent: "0.026",
        }],
      });
    }

    if (pathname === "/openapi/trade/order/list" && request.method === "GET") {
      return json({
        code: 0,
        data: [{
          orderId: "order-1",
    ...
    if (pathname === "/openapi/trade/order/detail" && request.method === "GET") {
      expect(searchParams.get("orderId")).toBe("order-1");
      return json({
        code: 0,
        data: {
          orderId: "order-1",
    ...
    if (pathname === "/openapi/trade/order/place" && request.method === "POST") {
      const body = JSON.parse(request.bodyText || "{}");
      expect(body.symbol).toBe("AAPL");
      return json({
        code: 0,
    ...
    if (pathname === "/openapi/trade/order/cancel" && request.method === "POST") {
      return json({ code: 0, data: { success: true } });
    }

    if (pathname === "/openapi/trade/order/cancel-all" && request.method === "POST") {
      return json({ code: 0, data: { success: true } });
    }

    if (pathname === "/openapi/market-data/stock/quotes" && request.method === "GET") {
      return json({
        code: 0,
        data: [{
          symbol: "AAPL",
