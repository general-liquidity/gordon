...
const LEGACY_SLASH_COMMANDS: SlashCommandSeed[] = [
  ...
  {
    name: "fisher",
    aliases: ["fisher-transform"],
    description: "Fisher Transform of a price series — sharper turning-point signals than RSI/stochastic",
    usage: "/fisher",
    category: "trading",
    level: 1,
    action: "tool",
    target: "compute_fisher_transform",
    whenToUse: "User wants a sharper turning-point oscillator at extremes",
  },
  {
    name: "hilbert",
    aliases: ["hilbert-transform", "phase"],
    description: "Ehlers's truncated Hilbert Transform — instantaneous cycle phase from a small price window",
    usage: "/hilbert",
    category: "trading",
    level: 1,
    action: "tool",
    target: "compute_hilbert_transform",
    whenToUse:
      "User wants to know whether the market is cycling or not — phase angle advances cleanly in a cyclic regime",
  },
  {
    name: "vidya",
    aliases: ["chande-ma"],
    description: "Chande's VIDYA — adaptive MA where smoothing varies with the stdev-fast / stdev-slow ratio",
    usage: "/vidya",
    category: "trading",
    level: 1,
    action: "tool",
    target: "compute_vidya",
    whenToUse:
      "User wants an adaptive MA alternative to KAMA based on relative volatility rather than trendiness",
  },
  {
    name: "trix",
    aliases: [],
    description: "Triple Exponential Smoothing rate-of-change — low-noise momentum filter",
    usage: "/trix",
    category: "trading",
    level: 1,
    action: "tool",
    target: "compute_trix",
    whenToUse:
      "User wants smooth momentum with reduced lag — signal-line crossover generates entries",
  },
  {
    name: "divergence-index",
    aliases: ["di"],
    description: "Appel's volatility-adjusted MACD with stdev-scaled bands that self-adjust to vol regimes",
    usage: "/divergence-index",
    category: "trading",
    level: 1,
    action: "tool",
    target: "compute_divergence_index",
    whenToUse:
      "User wants MACD-style signals with bands that adapt to volatility instead of fixed thresholds",
  },
  {
    name: "force-index",
    aliases: ["force"],
    description: "Elder's Force Index — volume × price change, EMA-smoothed; mid-frame oscillator for Triple Screen",
    usage: "/force-index",
    category: "trading",
    level: 1,
    action: "tool",
    target: "compute_force_index",
    whenToUse: "User wants a volume-conviction-weighted momentum reading",
  },
  {
    name: "elder-ray",
    aliases: ["ray"],
    description: "Elder-Ray Bull Power / Bear Power around an EMA centerline",
    usage: "/elder-ray",
    category: "trading",
    level: 1,
    action: "tool",
    target: "compute_elder_ray",
    whenToUse:
      "User wants to see the balance of bull-vs-bear pressure either side of a trend EMA",
  },
  {
    name: "kst",
    aliases: ["know-sure-thing"],
    description: "Pring's KST — composite momentum across four ROC horizons",
    usage: "/kst",
    category: "trading",
    level: 1,
    action: "tool",
    target: "compute_kst_index",
    whenToUse:
      "User wants intermediate-horizon momentum with high-frequency noise filtered out",
  },
  {
    name: "replicate-fund",
    aliases: ["replicate"],
    description: "Hedge-fund replication via OLS — recover allocation weights that mimic a target return series",
    usage: "/replicate-fund",
    category: "trading",
    level: 1,
    action: "tool",
    target: "replicate_hedge_fund",
    whenToUse:
      "User wants to reverse-engineer an external fund/strategy's holdings from its return track",
  },
  {
    name: "seasonality",
    aliases: ["seasonal"],
    description: "Aggregate daily returns by month and weekday with 95% confidence intervals",
    usage: "/seasonality",
    category: "trading",
    level: 1,
    action: "tool",
    target: "compute_seasonal_pattern",
    whenToUse:
      "User wants to identify calendar bias filters (Santa rally, Monday effect, holiday-week patterns)",
  },
  {
    name: "mesa",
    aliases: ["spectrum", "burg"],
    description: "Maximum Entropy Spectral Analysis — recover cycle structure from small samples via Burg AR fit",
    usage: "/mesa",
    category: "trading",
    level: 1,
    action: "tool",
    target: "compute_mesa_spectrum",
    whenToUse:
      "User wants short-term cycle detection where Fourier methods need too many samples",
  },
  {
    name: "orders",
    aliases: ["o"],
  ...
  // Radar — proactive mode (unsolicited suggestions from Gordon's observer loop)
  {
    ...
    subcommandDescriptions: {
      on: "Activate radar — starts the event-bus observer and 9 candidate producers",
  ...
  // Learn radar — walkthrough tutorial for proactive mode
  {
    name: "learn-radar",
