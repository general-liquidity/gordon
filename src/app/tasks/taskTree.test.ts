...
describe("taskTree", () => {
  test("creates a request root and routing node", () => {
    ...
    expect(
      tree.nodes.some((node) => node.kind === "request" && node.label === "Analyze Market"),
    ).toBe(true);
    expect(tree.nodes.some((node) => node.kind === "routing" && node.status === "running")).toBe(
      true,
    );
  });
  ...
  test("classifies market-analysis tools under a functional family", () => {
    ...
    expect(
      tree.nodes.some((node) => node.kind === "family" && node.label === "Market analysis"),
    ).toBe(true);
    expect(tree.nodes.some((node) => node.kind === "agent" && node.label === "Analyst")).toBe(true);
    expect(tree.nodes.some((node) => node.kind === "tool" && node.label === "Analyze Coin")).toBe(
      true,
    );
  });
  ...
  test("classifies systematic tools under systematic research", () => {
    ...
    expect(
      tree.nodes.some((node) => node.kind === "family" && node.label === "Systematic research"),
    ).toBe(true);
  });
  ...
  test("queues and dequeues follow-ups", () => {
    ...
    expect(tree.nodes.some((node) => node.kind === "queue" && node.status === "completed")).toBe(
      true,
    );
  });
  ...
  test("settles tool nodes on completion and cancellation", () => {
    let tree = createTaskTree({
      input: "/preview-order AAPL buy 10",
      actionId: "trading.preview_market_order",
    });
    tree = recordTaskTreeToolStart(
      tree,
    ...
    expect(tree.nodes.some((node) => node.kind === "tool" && node.status === "completed")).toBe(
      true,
    );
  ...
  test("deduplicates repeated tool-start events while the same tool is already running", () => {
    ...
    expect(
      tree.nodes.filter((node) => node.kind === "tool" && node.label === "Start Autonomous Mode"),
    ).toHaveLength(1);
  });
});
