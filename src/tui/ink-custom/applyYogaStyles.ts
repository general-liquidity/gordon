// applyYogaStyles — maps a Styles object onto a Yoga node.
//
// Status: Phase 1+ (behind GORDON_CUSTOM_RENDER flag). Mirrors Ink's
// `styles.js` one-to-one. Each applyXxx() is independent — only the props
// present in `style` are applied, so partial updates work for reconciler
// commitUpdate() deltas.
//
// Pulled out of `domRuntime.ts` so it can be tested without a full Yoga
// tree walk.
...
function applyMarginStyles(node: YogaNode, style: PartialStyles): void {
...
function applyPaddingStyles(node: YogaNode, style: PartialStyles): void {
...
function applyFlexStyles(node: YogaNode, style: PartialStyles): void {
...
function applyDisplayStyles(node: YogaNode, style: PartialStyles): void {
  if ("display" in style) {
    node.setDisplay(style.display === "flex" ? Yoga.DISPLAY_FLEX : Yoga.DISPLAY_NONE);
  }
}

function applyBorderStyles(node: YogaNode, style: PartialStyles): void {
  if ("borderStyle" in style) {
    const borderWidth = style.borderStyle ? 1 : 0;
    if (style.borderTop !== false) node.setBorder(Yoga.EDGE_TOP, borderWidth);
    if (style.borderBottom !== false) node.setBorder(Yoga.EDGE_BOTTOM, borderWidth);
    if (style.borderLeft !== false) node.setBorder(Yoga.EDGE_LEFT, borderWidth);
    if (style.borderRight !== false) node.setBorder(Yoga.EDGE_RIGHT, borderWidth);
  }
}

function applyGapStyles(node: YogaNode, style: PartialStyles): void {
...
export function applyYogaStyles(node: YogaNode, style: PartialStyles = {}): void {
...
export default applyYogaStyles;
