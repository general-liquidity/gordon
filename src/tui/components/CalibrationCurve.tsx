...
export function CalibrationCurve({ buckets, isWellCalibrated, overconfidenceScore, totalPredictions, overallAccuracy }: Props) {
  const status = isWellCalibrated ? "WELL CALIBRATED" :
    overconfidenceScore > 0.1 ? "OVERCONFIDENT" :
    overconfidenceScore < -0.1 ? "UNDERCONFIDENT" : "CALIBRATING";
    ...
    <Pane title="CONFIDENCE CALIBRATION">
      <Box flexDirection="column">
        <Box>
          ...
          <Text dimColor>  {"\u00B7"}  Overall accuracy: {(overallAccuracy * 100).toFixed(1)}%</Text>
          <Text dimColor>  {"\u00B7"}  N={totalPredictions}</Text>
        </Box>
        <Box marginTop={1} flexDirection="column">
          <Box>
            <Text dimColor bold>{"CONF".padEnd(6)}</Text>
            <Text dimColor bold>{"ACTUAL".padEnd(10)}</Text>
            <Text dimColor bold>{"EXPECTED".padEnd(10)}</Text>
            <Text dimColor bold>{"N".padEnd(6)}</Text>
            <Text dimColor bold>DELTA</Text>
          </Box>
          {buckets.map((b) => {
            const delta = b.accuracy - b.expectedAccuracy;
            const deltaColor = Math.abs(delta) < 0.1 ? "green" : Math.abs(delta) < 0.2 ? "yellow" : "red";
            return (
              <Box key={b.confidence}>
                ...
                <Text color={deltaColor}>{delta >= 0 ? "+" : ""}{(delta * 100).toFixed(1)}%</Text>
              </Box>
            );
          })}
        </Box>
