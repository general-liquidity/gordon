...
export const RiskCheckMessage = React.memo(function RiskCheckMessage({
  message,
}: {
  message: Message;
}) {
  const isPassing =
    message.content.includes("PASS") ||
      ...
      <Box paddingLeft={2} flexDirection="column">
        {message.content.split("\n").map((line, i) => {
          ...
          return (
            <Text
              key={i}
              color={isFail ? "red" : isPass ? "green" : undefined}
              dimColor={!isFail && !isPass}
            >
              {line}
            </Text>
          );
