...
export interface Plugin { id: string; name: string; description: string; version: string; installed: boolean; enabled: boolean; author?: string; }
interface Props { plugins: Plugin[]; onInstall?: (id: string) => void; onToggle?: (id: string) => void; onClose: () => void; }
    ...
    <Box flexDirection="column" borderStyle="round" borderColor="magenta" paddingX={1}>
      ...
      {plugins.map((p, i) => (
        <Box key={p.id}>
          ...
          <Text color={p.enabled ? "green" : p.installed ? "gray" : "cyan"}>{p.enabled ? "\u25CF" : p.installed ? "\u25CB" : "\u25C7"} </Text>
          <Text bold={i === selected}>{p.name}</Text>
          <Text dimColor> v{p.version}</Text>
          {p.author ? <Text dimColor> by {p.author}</Text> : null}
        </Box>
      ))}
      <Text dimColor>Enter to toggle {"\u00B7"} i to install {"\u00B7"} Esc to close</Text>
    </Box>
  );
}
