...
export function ElicitationDialog({ fields, onSubmit, onCancel, title }: Props) {
  ...
  useInput((input, key) => {
    if (key.escape) onCancel();
      ...
      if (field.type === "text" || field.type === "number") {
        if (key.backspace)
          setValues((p) => ({ ...p, [field.id]: String(p[field.id] ?? "").slice(0, -1) }));
        ...
        if (key.upArrow)
          setValues((p) => ({
            ...p,
            [field.id]: field.options![(curr - 1 + field.options!.length) % field.options!.length],
          }));
        if (key.downArrow)
          setValues((p) => ({
            ...p,
            [field.id]: field.options![(curr + 1) % field.options!.length],
          }));
      }
    }
  });
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor="cyan" paddingX={1}>
      ...
      <Text dimColor>
        Tab to next {"\u00B7"} Enter to submit {"\u00B7"} Esc to cancel
      </Text>
    </Box>
  );
}
