...
export function AlphaDecayChart({ months, decayPoint, onClose }: Props) {
  ...
  for (let row = chartHeight - 1; row >= 0; row--) {
    ...
    for (let col = 0; col < months.length; col++) {
      ...
      if (row < filled) {
        const sparkIdx = Math.min(
          SPARK_CHARS.length - 1,
          Math.max(0, Math.round((normalized - row) * (SPARK_CHARS.length - 1))),
        );
      ...
      <Box flexDirection="column" paddingLeft={2}>
        {chartRows.map((row, rowIdx) => {
          ...
          return (
            <Box key={rowIdx}>
              <Box width={6}>
                <Text dimColor>{isZeroLine ? " 0.00" : "     "}</Text>
              </Box>
        ...
        <Box>
          ...
          <Text dimColor>
            ...
            {months.length > 2
              ? " ".repeat(
                  Math.max(1, Math.floor(months.length / 2) - (months[0]?.month.length ?? 0)),
