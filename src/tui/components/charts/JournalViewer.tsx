...
export function JournalViewer({ trades, onCancel }: Props) {
  ...
  const filters: Array<"all" | "open" | "closed" | "winners" | "losers"> = [
    "all",
    "open",
    "closed",
    "winners",
  ...
  const winRate =
    ...
    Math.max(1, filtered.filter((t) => t.status === "closed").length);
        ...
        {filtered.slice(0, 15).map((trade, i) => {
          ...
          return (
            <Box key={trade.id}>
              ...
              <Text color={trade.side === "BUY" ? "green" : "red"} bold={isFocused}>
                {" "}
                {trade.side.padEnd(4)}
              </Text>
              ...
              <Text dimColor={!isFocused}>
                {" "}
                {String(trade.quantity).padEnd(10)} @ ${trade.price.toFixed(2).padEnd(10)}
              </Text>
              {trade.pnl !== undefined && (
                <Text color={pnlColor}>
                  {" "}
