...
interface Props { usedTokens: number; maxTokens: number; onCompact?: () => void; }
...
export function TokenWarning({ usedTokens, maxTokens }: Props) {
  ...
  const msg = pct >= 95 ? "AUTO-COMPACT imminent" : pct >= 80 ? "run /compact soon" : "consider /compact";

  return (
    <Box>
      <Text color={color} bold={pct >= 95}>{"\u26A0"} Context {Math.round(pct)}% </Text>
