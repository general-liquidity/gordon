...
export function PlanEditor({ plan, onSave, onCancel, onApprove }: Props) {
  ...
  const fields: Array<{ key: FieldKey; label: string; value: number; format: (n: number) => string }> = [
    { key: "entry", label: "Entry", value: draft.entry, format: (n) => `$${n.toFixed(2)}` },
    { key: "stopLoss", label: "Stop Loss", value: draft.stopLoss, format: (n) => `$${n.toFixed(2)}` },
    { key: "takeProfit", label: "Take Profit", value: draft.takeProfit, format: (n) => `$${n.toFixed(2)}` },
    { key: "sizeUsd", label: "Size (USD)", value: draft.sizeUsd, format: (n) => `$${n.toFixed(0)}` },
  ];
  ...
  useInput((input, key) => {
    ...
    else if (input === "e") { setIsEditing(true); setEditValue(String(draft[focused])); }
    ...
    <Pane title={`EDIT PLAN — ${draft.symbol} ${draft.side}`} color="yellow">
      <Box flexDirection="column">
        ...
        <Box marginTop={1}>
          ...
          <Text bold color={rr >= 2 ? "green" : rr >= 1 ? "yellow" : "red"}>{rr.toFixed(1)}:1</Text>
        </Box>
        <Box marginTop={1}>
