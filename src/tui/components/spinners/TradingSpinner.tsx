...
import { formatElapsed } from "../../hooks/animation/useElapsedTime.js";
...
function getToolVerb(toolName: string): string | null {
  ...
  if (/get_revenue_estimates/i.test(toolName)) return "Loading revenue estimates";
  if (/get_earnings_surprises/i.test(toolName)) return "Loading earnings surprises";
  if (/get_economic_calendar/i.test(toolName)) return "Loading macro calendar";
  if (/get_ipo_calendar/i.test(toolName)) return "Loading IPO calendar";
  if (/get_insider_transactions/i.test(toolName)) return "Loading insider trades";
  if (/get_insider_sentiment/i.test(toolName)) return "Loading insider sentiment";
  ...
  if (/get_price_target/i.test(toolName)) return "Loading price target";
  if (/get_upgrade_downgrade/i.test(toolName)) return "Loading rating changes";
  ...
  if (/get_social_sentiment/i.test(toolName)) return "Loading social sentiment";
  if (/get_etf_holdings/i.test(toolName)) return "Loading ETF holdings";
  if (/get_etf_profile/i.test(toolName)) return "Loading ETF profile";
  if (/get_etf_country_exposure/i.test(toolName)) return "Loading ETF country exposure";
  if (/get_etf_sector_exposure/i.test(toolName)) return "Loading ETF sector exposure";
  if (/get_mutual_fund_profile/i.test(toolName)) return "Loading fund profile";
  if (/get_mutual_fund_holdings/i.test(toolName)) return "Loading fund holdings";
  if (/get_mutual_fund_country_exposure/i.test(toolName)) return "Loading fund country exposure";
  if (/get_mutual_fund_sector_exposure/i.test(toolName)) return "Loading fund sector exposure";
  if (/get_company_profile/i.test(toolName)) return "Loading company profile";
  if (/get_basic_financials/i.test(toolName)) return "Loading financials";
  if (/get_financials_reported/i.test(toolName)) return "Loading reported financials";
  if (/get_peer_companies/i.test(toolName)) return "Loading peer companies";
  if (/get_dividends/i.test(toolName)) return "Loading dividend history";
  if (/get_stock_splits/i.test(toolName)) return "Loading split history";
  if (/get_fund_ownership/i.test(toolName)) return "Loading fund ownership";
  if (/get_institutional_ownership/i.test(toolName)) return "Loading institutional ownership";
  if (/get_lobbying/i.test(toolName)) return "Loading lobbying records";
  if (/get_usa_spending/i.test(toolName)) return "Loading government contracts";
  if (/get_uspto_patents/i.test(toolName)) return "Loading patent filings";
  if (/get_visa_applications/i.test(toolName)) return "Loading visa applications";
  if (/get_supply_chain/i.test(toolName)) return "Loading supply chain";
  if (/get_esg_score/i.test(toolName)) return "Loading ESG score";
  if (/list_earnings_transcripts/i.test(toolName)) return "Listing transcripts";
  if (/get_earnings_transcript/i.test(toolName)) return "Loading transcript";
  if (/get_stock_quote/i.test(toolName)) return "Loading stock quote";
  if (/get_stock_candles/i.test(toolName)) return "Loading stock candles";
  if (/get_stock_symbols/i.test(toolName)) return "Loading symbol list";
  if (/symbol_lookup/i.test(toolName)) return "Searching symbols";
  if (/get_market_status/i.test(toolName)) return "Checking market status";
  if (/get_company_news/i.test(toolName)) return "Loading company news";
  if (/get_market_news/i.test(toolName)) return "Loading market news";
  if (/get_pattern_recognition/i.test(toolName)) return "Scanning chart patterns";
  if (/get_support_resistance/i.test(toolName)) return "Loading S/R levels";
  if (/get_aggregate_signal/i.test(toolName)) return "Loading aggregate signal";
  if (/get_index_constituents/i.test(toolName)) return "Loading index members";
  if (/get_bond_yield_curve/i.test(toolName)) return "Loading yield curve";
  if (/get_bond_profile/i.test(toolName)) return "Loading bond profile";
  if (/get_finnhub_crypto_exchanges/i.test(toolName)) return "Loading crypto exchanges";
  if (/get_finnhub_crypto_symbols/i.test(toolName)) return "Loading crypto symbols";
  if (/get_finnhub_crypto_candles/i.test(toolName)) return "Loading crypto candles";
  if (/get_finnhub_crypto_profile/i.test(toolName)) return "Loading crypto profile";
  if (/get_forex_rates/i.test(toolName)) return "Loading forex rates";
  if (/list_economic_codes/i.test(toolName)) return "Loading economic codes";
  if (/get_economic_data/i.test(toolName)) return "Loading economic series";
  ...
  if (/dexscreener|dex_pair|search_dex/i.test(toolName)) return "Searching DEX";
  if (/birdeye/i.test(toolName)) return "Querying Birdeye";
  if (/codex|defined/i.test(toolName)) return "Querying Codex";
  if (/oneinch|1inch/i.test(toolName)) return "Querying 1inch";
  if (/coingecko.?onchain/i.test(toolName)) return "Fetching onchain OHLCV";
  if (/nansen/i.test(toolName)) return "Querying Nansen";
  if (/arkham/i.test(toolName)) return "Querying Arkham";
  if (/covalent|goldrush/i.test(toolName)) return "Querying Covalent";
  if (/moralis/i.test(toolName)) return "Querying Moralis";
  if (/zerion/i.test(toolName)) return "Querying Zerion";
