...
export interface LoopTask { id: string; command: string; intervalMs: number; lastRunAt: number; nextRunAt: number; runCount: number; status: "running" | "paused" | "stopped"; }
export interface BatchTask { id: string; commands: string[]; results: any[]; status: "running" | "completed" | "failed"; }
...
export class SkillRunner {
  ...
  runLoop(command: string, intervalMs: number): LoopTask {
    ...
    const task: LoopTask = { id, command, intervalMs, lastRunAt: 0, nextRunAt: Date.now() + intervalMs, runCount: 0, status: "running" };
  ...
  runBatch(commands: string[]): BatchTask {
    ...
    setTimeout(() => { task.status = "completed"; }, 1000);
    return task;
  }

  stopLoop(taskId: string): void {
    const entry = this.loops.get(taskId);
    if (entry) { clearInterval(entry.timer); entry.task.status = "stopped"; this.loops.delete(taskId); }
  }

  stopBatch(taskId: string): void { this.batches.delete(taskId); }
