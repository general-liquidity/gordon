...
describe("renderBootStaticRows", () => {
  test("renders padded labels and wide-mode hints", () => {
    ...
    expect(stripAnsi(rows[0]!)).toContain("  model     claude-sonnet-4-6 high");
    expect(stripAnsi(rows[0]!)).toContain("/model to change");
    expect(stripAnsi(rows[1]!)).toContain("  mode      ask");
    expect(stripAnsi(rows[1]!)).toContain("/auto to change");
  });

  test("drops hints at narrow widths before truncating values", () => {
    const rows = renderBootStaticRows(fixture(), 65);
    expect(stripAnsi(rows[0]!)).not.toContain("/model to change");
    expect(stripAnsi(rows[1]!)).not.toContain("/auto to change");
    expect(visibleLength(rows[0]!)).toBeLessThanOrEqual(64);
  });
  ...
  test("renders paper mode with loud badge and live-exit hint", () => {
    ...
    expect(stripAnsi(rows[1]!)).toContain("paper [PAPER]");
    expect(stripAnsi(rows[1]!)).toContain("/live to exit");
  });
  ...
  test("renders firm and scoped kill-switch summaries", () => {
    ...
    expect(stripAnsi(firm[5]!)).toContain("HALTED: firm — test halt rationale");
    ...
    expect(stripAnsi(scoped[5]!)).toContain("tripped: venue:binance, strategy:s1");
  });
  ...
  test("renders guard failure states", () => {
    ...
    const guardRow = stripAnsi(rows[4]!);
  ...
  test("renders radar off, warming, and aged variants", () => {
    ...
    }), 120)[6]!)).toContain("off — set GORDON_PROACTIVE_AUTO_START=1 or /radar start");
    expect(stripAnsi(renderBootStaticRows(fixture({
      radar: { running: true, producerCount: 21, lastCardAgeMs: null },
    }), 100)[6]!)).toContain("21 producers · warming up");
    expect(stripAnsi(renderBootStaticRows(fixture({
      radar: { running: true, producerCount: 21, lastCardAgeMs: 2 * 60 * 60 * 1000 },
    }), 100)[6]!)).toContain("21 producers · last card 2h ago");
  });
