...
import { registerContextInvalidator } from "../../infra/runtime/credentialRefresh.ts";
...
export async function initializeRuntime(setState: StateUpdater): Promise<SessionRuntime> {
  ...
  try {
    ...
    if (
      resolveFlag("GORDON_AUTODREAM_ENABLED") !== "0" &&
      resolveFlag("GORDON_AUTODREAM_ENABLED") !== "false"
    ) {
  ...
  try {
    ...
    // Read via resolveFlag so settings.json + /flags can toggle it (env still wins).
    if (
      resolveFlag("GORDON_REFLECTION_ENABLED") !== "0" &&
      resolveFlag("GORDON_REFLECTION_ENABLED") !== "false"
    ) {
    ...
    console.error(
      "[runtime] tradeReflection init failed:",
  ...
  const sessionPromise = activeRuntime
    .initializeSession({ autoResume: true })
    .catch((err: unknown) => {
  ...
  const toolingPromise = activeRuntime
    .initializeTooling({ enableHotReload: true })
    .catch((err: unknown) => {
      const msg = err instanceof Error ? err.message : String(err);
...
export async function shutdownRuntime(
  _reason: "user_quit" | "timeout" | "error" | "graceful" | "external_signal" = "graceful",
): Promise<void> {
  abortActiveTurn();
...
export async function handleInput(input: string, setState: StateUpdater): Promise<void> {
  const runtime = activeRuntime;
  if (!runtime) return;
...
async function handleMenuCommand(
  ...
  if (await handleAutonomousMenuCommand(resolved, args, setState, runtime, command, streamResponse))
    return;
      ...
      switch (event.type) {
        case "text_delta": {
          ...
          if (chunk) {
            if (
              lastEventWasToolEnd &&
              textFlusher.buffer.length > 0 &&
              !textFlusher.buffer.endsWith("\n")
            ) {
              textFlusher.push("\n\n");
            }
          ...
          if (event.agentName) {
            ...
            setState((prev: any) => {
              ...
              const updatedChains = prev.activeAgents.map((c: any) =>
                c.status === "running"
                  ? { ...c, status: "done", duration: Date.now() - c.startedAt }
                  : c,
              );
              // Add new chain
              const newChain = {
        ...
        case "tool_call_start":
          if (event.toolName) {
            taskTree =
              recordTaskTreeToolStart(taskTree, event.toolName, event.toolArgs) ?? taskTree;
            setState((prev: any) => {
        ...
        case "tool_call_end":
          ...
          setState((prev: any) => {
            ...
            const updatedCalls = completeFirstRunningCall(
              ...
              event.toolName,
              {
                ...
                duration:
                  Date.now() -
                  ((prev.activeToolCalls ?? []).find(
                    (tc: any) => tc.toolName === event.toolName && tc.status === "running",
                  )?.startedAt ?? Date.now()),
              },
            );
          ...
          for (const approval of rawPending) {
            try {
              ...
              const quickResult = quickPermissionCheck(
                approvalRulesToQuickCheckRules(
                  runtime.listApprovalRules(),
          ...
          setState((prev: any) => {
            ...
            return {
              ...
              completedMessageCount:
                prev.messages.length + riskMessages.length + approvalMsgs.length,
              streamBuffer: "",
              isStreaming: false,
              activeAgents: [],
              activeToolCalls: [],
              ...
              contextTokens:
                inputTokens > 0 ? inputTokens : (prev.contextTokens ?? 0) + totalTokens,
...
function looksLikeApprovalId(candidate: string, runtime: SessionRuntime): boolean {
  ...
  try {
    ...
    const pending =
      (state as { approvals?: { pending?: Array<{ id: string }> } }).approvals?.pending ?? [];
...
async function handleApproval(
  ...
  if (!requestId) {
    addMessage(
      setState,
      "system",
      `Usage: ${action} <id> ${action === "deny" ? "[reason]" : "[persist]"}`,
    );
    return;
  }
  ...
  try {
    if (action === "approve") {
      ...
      addMessage(
        ...
        "gordon",
        result
          ? `\u2713 Approved: ${result.toolName}${persist ? " (persisted)" : ""}`
          : `No pending approval: ${requestId}`,
        "approval",
      );
    } else {
      const persist = rest.includes("persist");
      ...
      addMessage(
        ...
        "gordon",
        result
          ? `\u2717 Denied: ${result.toolName}${reason ? ` (${reason})` : ""}`
          : `No pending approval: ${requestId}`,
        "approval",
      );
    }
...
export function handleApprovalDecision(
  ...
  if (decision === "modify") {
    ...
    runtime.approvePendingRequest(approvalId, {
      persist,
      actor: "user",
      scope: persist ? "session" : undefined,
    });
    addMessage(
      setState,
      "gordon",
      `\u2713 Approved${persist ? " (always for this tool)" : ""}.`,
      "approval",
    );
  }
  ...
  _bgMonitorInterval = setInterval(() => {
    ...
    for (const task of tasks) {
      if (task.status === "completed" || task.status === "failed") {
        ...
        const _color = task.status === "completed" ? "green" : "red";
