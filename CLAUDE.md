...
| `GORDON_AGENT_READINESS_GATE=1` | Adds boot-time readiness rows to the doctor report (`infra/diagnostics/agentReadiness.ts` via `app/setup/harness-checks.ts`). NOT a gate: nothing blocks agent spawn on a failing condition, and `GORDON_AGENT_READINESS_OVERRIDE=1` suppresses the rows rather than overriding a block. |
| `GORDON_RISK_ACK=1` | Anti-rubber-stamp risk-acknowledgement gate (`infra/safety/anti-trap/riskAcknowledgement.ts`): on medium+ tier `execute_plan`, the agent must enumerate the top risk dimensions / gate warnings in `acknowledgedRisks` before execution proceeds. Opt-in — forces explicit supervision instead of single-keystroke approval. |
